...
export async function GET(
  ...
  try {
    ...
    const response = await nestServer.get(
      `/kanban/projects/${projectId}/sprints/${sprintId}/board`,
      {
    ...
    console.error('Error fetching kanban board by sprint:', error);
    return NextResponse.json(
      {
        error:
          error.response?.data?.message || 'Failed to fetch kanban board',
      },
